...
import { X, Phone, User, MapPin, Clock, Activity, Heart, Users, NavigationArrow } from 'phosphor-react';
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 bg-gradient-to-r from-purple-600/20 to-pink-600/20 border-b border-gray-700/50 p-6 backdrop-blur-xl">
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className={`inline-flex items-center gap-2 mt-2 px-3 py-1 rounded-full bg-${getStatusColor(ambulance.status)}-500/20 border border-${getStatusColor(ambulance.status)}-500/30`}>
              <div className={`w-2 h-2 rounded-full bg-${getStatusColor(ambulance.status)}-400 animate-pulse`}></div>
              <span className={`text-sm font-medium text-${getStatusColor(ambulance.status)}-400`}>
      ...
      <div className="p-6 space-y-6">
        ...
        {ambulance.driver && (
          <div className="bg-gradient-to-br from-gray-700/50 to-gray-800/50 border border-gray-600/50 rounded-xl p-4">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <div className="text-white font-medium">{ambulance.driver.name}</div>
                <div className="text-gray-400 text-sm">{ambulance.driver.phone}</div>
              </div>
            </div>
            <button
        ...
        {ambulance.currentEmergency?.patient && (
          <div className="bg-gradient-to-br from-gray-700/50 to-gray-800/50 border border-gray-600/50 rounded-xl p-4">
            ...
            {ambulance.currentEmergency.priority && (
              <div className={`inline-flex items-center gap-2 mb-3 px-3 py-1 rounded-full bg-${getPriorityColor(ambulance.currentEmergency.priority)}-500/20 border border-${getPriorityColor(ambulance.currentEmergency.priority)}-500/30`}>
                <div className={`w-2 h-2 rounded-full bg-${getPriorityColor(ambulance.currentEmergency.priority)}-400`}></div>
                <span className={`text-sm font-medium text-${getPriorityColor(ambulance.currentEmergency.priority)}-400 capitalize`}>
            ...
            <div className="space-y-2 mb-4">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-white">{ambulance.currentEmergency.patient.age || 'N/A'}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-400">Gender:</span>
                <span className="text-white capitalize">{ambulance.currentEmergency.patient.gender || 'N/A'}</span>
              </div>
              {ambulance.currentEmergency.condition && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">Condition:</span>
                  <span className="text-white">{ambulance.currentEmergency.condition}</span>
                </div>
              )}
            </div>
            ...
            {ambulance.currentEmergency.vitals && (
              <div className="bg-gray-800/50 rounded-lg p-3 space-y-2">
                ...
                {ambulance.currentEmergency.vitals.bloodPressure && (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Activity size={16} weight="duotone" className="text-blue-400" />
                ...
                {ambulance.currentEmergency.vitals.oxygenLevel && (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Activity size={16} weight="fill" className="text-green-400" />
                      <span className="text-xs text-gray-400">Oxygen Level</span>
                    </div>
                    <span className="text-sm text-white font-medium">
                      {ambulance.currentEmergency.vitals.oxygenLevel}%
                    </span>
          ...
          <div className="bg-gradient-to-br from-gray-700/50 to-gray-800/50 border border-gray-600/50 rounded-xl p-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-white">{ambulance.currentLocation.coordinates[1].toFixed(6)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Longitude:</span>
                <span className="text-white">{ambulance.currentLocation.coordinates[0].toFixed(6)}</span>
              </div>
              {ambulance.speed && (
                <div className="flex justify-between">
        ...
        {/* Actions */}
        {(ambulance.status === 'en-route' || ambulance.status === 'returning') && (
          <button
